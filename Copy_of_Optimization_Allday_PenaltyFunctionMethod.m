...
Vlimit12 = [-0.5, 0.5];               % 设置速度限制
WinPowV96N = Vlimit12(2)*rand(96,N)- Vlimit12(2)/2;                  % 初始种群的速度
SolPowV96N = Vlimit12(2)*rand(96,N)- Vlimit12(2)/2;                  % 初始种群的速度
BatPowV96N = Vlimit12(2)*rand(96,N) - Vlimit12(2)/2;              % 初始种群的速度

w = 2;                        % 惯性权重。%这里注意，看上面！我的速度设置的是一个0到1的随机数，但是实际上我的位置的变动的尺度可能是几百那么大，如此一来我的速度就非常小几乎可以忽略不记了。因此如果我想要达到一种比较好的效果，那么我就需要在这里设置一个系数。
c1 = 2;                       % 自我学习因子
c2 = 2;                       % 群体学习因子
...
WinPow96N = rand(96,N);%初始种群的位置
SolPow96N = rand(96,N);%初始种群的位置
BatPow96N = rand(96,N);%初始种群的位置
...
%     end
   ...
   for j = 1:N                                  %更新个体最小值
    ...
    WinPowV96N = WinPowV96N*w + c1*rand(96,N).*(WinPowMX96N - WinPow96N) + c2*rand(96,N).*(WinPowMXs961 - WinPow96N); 
    SolPowV96N = SolPowV96N*w + c1*rand(96,N).*(SolPowMX96N - SolPow96N) + c2*rand(96,N).*(SolPowMXs961 - SolPow96N); 
    BatPowV96N = BatPowV96N*w + c1*rand(96,N).*(BatPowMX96N - BatPow96N) + c2*rand(96,N).*(BatPowMXs961 - BatPow96N); 
...
%      BatPowRecord96Nger(:,:,iter) = BatPow96N;
     ...
     Amp = 10;
    if mod(iter,1000) == 0
         WinPow96N = WinPow96N -WinPowV96N+ Amp*rand(96,N);% 位置更新
         SolPow96N = SolPow96N -SolPowV96N+ Amp*rand(96,N);% 位置更新
         BatPow96N = BatPow96N -BatPowV96N+ Amp*rand(96,N);% 位置更新
    end   
     ...
     temp130 = repmat(BatPowLimit962(:,2),1,N);
     BatPow96N(BatPow96N > temp130) =  temp130(BatPow96N > temp130);
     temp130 = repmat(BatPowLimit962(:,1),1,N);
     BatPow96N(BatPow96N < temp130) =  temp130(BatPow96N < temp130);
